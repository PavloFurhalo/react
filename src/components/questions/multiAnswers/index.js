    ...
    {props.answers.map((answer, i) => {
     ...
     return (<div>
      <input
       ...
       checked =  {props.correctAnswer.includes(i)&& showCorrectAnswer ? true : false}
       
      />
      {props.correctAnswer.includes(i)&& showCorrectAnswer ? <label style={{color : "green"}} for={id}>{answer}</label> : <label for={id}>{answer}</label>}
     </div>);
    })}
   </div>
   <div className='check'>
